...
# CursoSchema é responsável por fazer essa serialização JSON -> API: response_model é o retorno.
@router.post('/', status_code=status.HTTP_201_CREATED, response_model=CursoSchema)
async def post_curso(curso: CursoSchema, db: AsyncSession = Depends(get_session)):
    novo_curso = CursoModel(titulo=curso.titulo, aulas=curso.aulas,
                            horas=curso.horas, professor=curso.professor)
...
async def put_curso(curso_id: int, curso: CursoSchema, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if curso_up:
            ...
            curso_up.professor  = curso.professor
